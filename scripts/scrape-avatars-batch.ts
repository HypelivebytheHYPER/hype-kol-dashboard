...
async function scrapeAvatar(page: any, handle: string): Promise<string | null> {
  try {
    await page.goto(`https://www.tiktok.com/@${handle}`, { waitUntil: 'domcontentloaded', timeout: 8000 });
...
(async () => {
  ...
  const batch = creators.slice(OFFSET, OFFSET + BATCH_SIZE);
  console.log(`Batch ${OFFSET}-${OFFSET + batch.length - 1} of ${creators.length}`);

  const browser = await chromium.launch({ headless: true });
  const page = await browser.newPage();
  const result: Record<string, { record_id: string; avatar_url: string }> = {};

  for (const c of batch) {
    const url = await scrapeAvatar(page, c.handle);
    if (url) result[c.handle.toLowerCase()] = { record_id: c.record_id, avatar_url: url };
  }

  await browser.close();

  const file = `./scripts/avatar-scrape-result.json`;
  const existing = fs.existsSync(file) ? JSON.parse(fs.readFileSync(file, 'utf8')) : {};
  fs.writeFileSync(file, JSON.stringify({ ...existing, ...result }, null, 2));
  console.log(`Saved ${Object.keys(result).length}/${batch.length} avatars (total: ${Object.keys({...existing, ...result}).length})`);
})();
