...
import {
  ...
  ANIMATION_DELAY_NORMAL_MS,
  ANIMATION_DELAY_MEDIUM_MS,
...
const R2 = SERVICES.r2Studio;

/* ── Utility: Intersection Observer for scroll animations ── */
function useInView(threshold = INTERSECTION_THRESHOLD) {
...
function Hero() {
  ...
  useEffect(() => {
    ...
    let id = setInterval(tick, HERO_CAROUSEL_INTERVAL_MS);
    const onVis = () => {
      clearInterval(id);
      if (!document.hidden) id = setInterval(tick, HERO_CAROUSEL_INTERVAL_MS);
    };
  ...
  return (
    <section className={`relative ${HERO.height} flex flex-col justify-end overflow-hidden`}>
      ...
      {/* Content */}
      <div className={`relative ${Z_INDEX.content} ${SECTION.paddingX} pb-20 md:pb-28`}>
        <Reveal>
          <div className="flex items-center gap-2 mb-6">
            <span className={cn(`inline-block w-2 h-2 ${RADIUS.full} animate-pulse`, STUDIO.bg)} />
        ...
        <Reveal delay={ANIMATION_DELAY_SHORT_MS}>
        ...
        <Reveal delay={ANIMATION_DELAY_MEDIUM_MS}>
        ...
        <Reveal delay={ANIMATION_DELAY_LONG_MS}>
          <div className="flex flex-wrap items-center gap-4">
            <Button
              variant="studio"
              size="lg"
              className={`${RADIUS.full} gap-2 px-7`}
              onClick={() =>
                document
            ...
            <Link href={ROUTES.KOLS}>
              <Button
                variant="outline"
                size="lg"
                className={cn(`${RADIUS.full} gap-2 text-foreground hover:text-foreground backdrop-blur-sm border-foreground/15`, FG_OPACITY.subtle)}
              >
                Browse Creators
                <ArrowRight className="size-4" />
              </Button>
    ...
    <section className={cn(SECTION.paddingX, SECTION.py)}>
      <div className={SECTION.container}>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 lg:gap-16 mb-16 md:mb-24">
          ...
          <Reveal delay={ANIMATION_DELAY_NORMAL_MS} className="flex items-end">
        ...
        <div className={`grid grid-cols-1 md:grid-cols-3 gap-px bg-border/30 ${RADIUS["2xl"]} overflow-hidden`}>
          {CAPABILITIES.map((cap, i) => (
            <Reveal key={cap.title} delay={i * ANIMATION_STAGGER_MS}>
              <div className={`bg-background p-8 md:p-10 h-full group hover:bg-muted/30 transition-colors ${DURATION.slow}`}>
                <div className={cn(
                  `mb-6 inline-flex size-12 items-center justify-center ${RADIUS.xl} border transition-transform ${DURATION.slow} group-hover:scale-110 group-hover:rotate-3`,
                  STUDIO.bgSubtle,
                  STUDIO.borderSubtle,
                  STUDIO.text
                )}>
...
function RecommendedBadge({ sm = false }: { sm?: boolean }) {
  return (
    <div className={cn(
      `absolute ${RADIUS.full} font-semibold flex items-center`,
      STUDIO.badge,
...
function PhotoCountBadge({ count, sm = false }: { count: number; sm?: boolean }) {
  return (
    <div className={cn(
      `absolute ${RADIUS.full} backdrop-blur-sm font-medium`,
      OVERLAY.heavy,
      TEXT_OPACITY.normal,
...
function StudioCard({ studio }: { studio: Studio }) {
  ...
  return (
    <Card className={cn(
      `relative shrink-0 overflow-hidden ${RADIUS.xl} bg-card/30 hover:${SHADOW.xl} group ${WIDTH.studioCard} snap-start border`,
      DURATION.slow,
      STUDIO.borderHover
    )}>
      {/* Lead Photo */}
      {leadImage && (
        <div className={`relative ${ASPECT.studio} overflow-hidden`}>
          <Image
            src={leadImage}
            alt={studio.name}
            fill
      ...
      <div className="p-5">
        ...
        {studio.reference && (
          <Button
            ...
            className={`w-full ${RADIUS.full} gap-2`}
...
function FeaturedStudio({ studio, images }: { studio: Studio; images: string[] }) {
  ...
  return (
    <div className={cn(`grid grid-cols-1 lg:grid-cols-2 gap-0 overflow-hidden ${RADIUS["2xl"]} bg-card/30 border`, BORDER_OPACITY.medium)}>
      {/* Image Side */}
      ...
      <div className="p-8 md:p-10 lg:p-12 flex flex-col justify-center">
        ...
        {studio.reference && (
          <Button
            variant="studio"
            size="lg"
            className={`${RADIUS.full} gap-2 px-7 w-fit`}
            onClick={() => window.open(studio.reference, "_blank")}
          >
            View Photos
            <ArrowRight className="size-4" />
...
function StudiosSection({ studios }: { studios: Studio[] }) {
  ...
  const rest = featured
    ...
    : studios;
  const featuredImages = HERO_IMAGES;
      ...
      <div className={cn(SECTION.paddingX, "mb-12 md:mb-16")}>
        <div className={SECTION.container}>
          <div className="flex flex-col md:flex-row md:items-end md:justify-between gap-6">
            ...
            <Reveal delay={ANIMATION_DELAY_NORMAL_MS}>
...
function VideoCard({
  ...
  return (
    <Card className={`relative overflow-hidden bg-background group border-0 ring-0 ${RADIUS.xl} shrink-0 w-full snap-start`}>
        ...
        {!playing && (
          <div className={cn("absolute inset-0 flex items-center justify-center", OVERLAY.subtle)}>
            <Button
              ...
              onClick={toggle}
              className={cn(`size-16 ${RADIUS.full} text-background hover:bg-foreground hover:scale-110 transition-all ${DURATION.normal} backdrop-blur-sm`, FG_OPACITY.heavy)}
            >
      ...
      <div className={cn(
        ...
        <div className="flex items-center gap-3">
          ...
          <div className={cn(`flex-1 h-1 ${RADIUS.full} overflow-hidden`, FG_OPACITY.light)}>
            <div
              className={cn(`h-full ${RADIUS.full} transition-all`, STUDIO.progress)}
  ...
  return (
    <section className={cn(SECTION.paddingX, SECTION.pyLg)}>
      <div className={SECTION.container}>
        <Reveal>
          <div className={cn(
            `relative overflow-hidden ${RADIUS["2xl"]} p-10 md:p-16 lg:p-20 border`,
