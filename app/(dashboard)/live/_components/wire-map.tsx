...
import { mcHue, oklch, canvasColors, CARD, RADIUS } from "@/lib/design-tokens";
import type { LiveMC } from "@/lib/types";

interface WireMapProps {
  mcs: LiveMC[];
  selectedCategory?: ContentCategoryId | null;
  onSelectMC?: (mcId: string) => void;
  onSelectCategory?: (catId: ContentCategoryId) => void;
...
export function WireMap({ mcs, selectedCategory, onSelectMC, onSelectCategory, onSelectBrand }: WireMapProps) {
  ...
  const handleClick = (e: React.MouseEvent<HTMLCanvasElement>) => {
    ...
    if (!hit) return;
    if (hit.startsWith("cat:")) {
      onSelectCategory?.(hit.slice(4) as ContentCategoryId);
    } else if (hit.startsWith("brand:")) {
      onSelectBrand?.(hit.slice(6));
    } else {
      onSelectMC?.(hit);
    }
  };
    ...
    if (e.key === "Enter" || e.key === " ") {
      ...
      if (currentId) {
        ...
        if (currentId.startsWith("cat:")) {
          onSelectCategory?.(currentId.slice(4) as ContentCategoryId);
        } else if (currentId.startsWith("brand:")) {
          onSelectBrand?.(currentId.slice(6));
        } else {
          onSelectMC?.(currentId);
        }
      } else {
  ...
  return (
    <div ref={containerRef} className={CARD.base}>
      <div className="px-4 py-3 border-b border-border flex items-center justify-between">
        ...
        <div className="flex gap-3 text-2xs text-muted-foreground">
          <span className="flex items-center gap-1.5">
            <span className={`size-2.5 ${RADIUS.full} inline-block bg-gradient-to-br from-chart-3 to-chart-4`} /> MC
          </span>
          <span className="flex items-center gap-1.5">
            <span className={`size-2.5 ${RADIUS.full} inline-block bg-chart-2`} /> Category
          </span>
          <span className="flex items-center gap-1.5">
            <span className={`size-2.5 ${RADIUS.full} inline-block bg-muted-foreground`} /> Brand
          </span>
        </div>
      </div>
      <canvas
