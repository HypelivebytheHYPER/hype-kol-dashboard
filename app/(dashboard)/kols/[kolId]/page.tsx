...
import { useKOL, useKOLRelated } from "@/hooks/use-kols";
import {
  ExternalLink,
  Mail,
  Phone,
...
import { AddToCampaignDialog } from "@/components/selection/add-to-campaign-dialog-dynamic";
...
export default function KOLProfilePage({ params }: { params: Promise<{ kolId: string }> }) {
  const { kolId } = use(params);
  const { data: kolData, isLoading, error } = useKOL(kolId);

  if (isLoading) return <ProfileSkeleton />;

  if (error || !kolData?.data) {
    return (
      <div className="flex flex-col items-center justify-center py-20 gap-4">
        <p className="text-muted-foreground">KOL not found</p>
        <Link href="/kols">
  ...
  const urls = (kol as unknown as Record<string, unknown>).urls as
        ...
        <CardContent className="px-6 pb-6 -mt-12 relative">
          <div className="flex flex-col sm:flex-row sm:items-end sm:justify-between gap-4">
            ...
            <div className="flex items-center gap-2 flex-wrap pb-1">
              ...
              <AddToCampaignButton kolId={kol.id} kolName={kol.name} />
            </div>
          </div>
...
function LinkedAccountsSection({ kol }: { kol: ApiKOL }) {
  const { data: relatedData } = useKOLRelated(kol.id);
  const parentKol = relatedData?.data?.parent;
  const childKols = relatedData?.data?.children || [];
...
function AddToCampaignButton({ kolId, kolName }: { kolId: string; kolName: string }) {
  ...
  return (
    <>
      ...
      <AddToCampaignDialog
        ...
        preselectedKOLs={[{ id: kolId, name: kolName, handle: "" }]}
      />
    </>
  );
}
