...
import { ROUTES, RADAR_CEILINGS, ENGAGEMENT_COUNT_THRESHOLD, TOOLTIP_DELAY_MS, getTierColor } from "@/lib/constants";
import { cn } from "@/lib/cn";
import { VALUE_ACCENT, SHADOW, RADIUS, WIDTH } from "@/lib/design-tokens";
import {
  ExternalLink,
  Mail,
  Phone,
        ...
        <CardContent className="px-6 pb-6 -mt-12 relative">
          <div className="flex flex-col sm:flex-row sm:items-end sm:justify-between gap-4">
            <div className="flex items-end gap-4">
              <div className={`relative border-4 border-card ${SHADOW.xl} size-24 ${RADIUS.full} overflow-hidden bg-muted flex items-center justify-center text-2xl font-semibold`}>
                {kol.image ? (
                  <img
                    src={kol.image}
      ...
      <div className="flex items-center gap-1.5 text-muted-foreground mb-0.5">
        ...
        {hint && (
          <Tooltip>
            ...
            <TooltipContent side="top" className={`${WIDTH.tooltipMax} text-xs leading-relaxed`}>
              {hint}
            </TooltipContent>
          </Tooltip>
        )}
...
function StatTile({ label, value }: { label: string; value: string }) {
  return (
    <div className={`p-3 ${RADIUS.lg} bg-muted/40`}>
...
function ContactRow({
  ...
  return (
    <div className={`flex items-center gap-3 p-3 ${RADIUS.lg} bg-muted/30`}>
