...
import { useKOLs, useLiveSellers } from "@/hooks";
...
export default function KOLsPage() {
  ...
  // Only block render on KOLs — sellers lazy-merge in when ready
  const isLoading = kolsLoading;

  // Merge KOLs and Live Sellers - computedImageUrl already provided by API
  const allKOLs = useMemo(() => {
    const kols: ApiKOL[] = [];
    const seen = new Set<string>();
    if (sourceTab === "all" || sourceTab === "kols") {
      for (const k of kolsData?.data || []) {
        if (!seen.has(k.handle)) {
          seen.add(k.handle);
          kols.push(k);
        }
      }
    }
    if (sourceTab === "all" || sourceTab === "live_sellers") {
      for (const s of sellersData?.data || []) {
        if (!seen.has(s.handle)) {
          seen.add(s.handle);
          kols.push(s as unknown as ApiKOL);
        }
      }
    }
    return kols;
  }, [kolsData, sellersData, sourceTab]);
  ...
  const filtered = useMemo(() => {
    ...
    if (selectedTypes.length > 0 && !smartFilters.kolType) {
      result = result.filter((k) => selectedTypes.includes(k.kolType));
    }

    return result;
  }, [allKOLs, smartFilters, selectedTiers, selectedPlatforms, selectedTypes]);
  ...
  const activeFilterCount = selectedTiers.length + selectedPlatforms.length + selectedTypes.length;
      ...
      <div className="flex flex-col sm:flex-row sm:items-end sm:justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-muted-foreground mt-1">
            Discover, filter and shortlist influencers for your campaigns
          </p>
        </div>
        <div className="flex items-baseline gap-4 sm:text-right">
          <Stat
            label="Database"
            value={formatNumber((kolsData?.total || 0) + (sellersData?.total || 0))}
            suffix={sellersLoading ? "+" : undefined}
          />
        ...
        <div className="flex flex-col sm:flex-row gap-3">
          ...
          >
            <TabsList className="h-9">
              ...
              <TabsTrigger value="kols" className="text-xs px-3">
                KOLs
              </TabsTrigger>
              <TabsTrigger value="live_sellers" className="text-xs px-3 gap-1.5">
                Live Sellers
                {sellersLoading && <Loader2 className="w-3 h-3 animate-spin opacity-50" />}
              </TabsTrigger>
            </TabsList>
          </Tabs>
            ...
            {activeFilterCount > 0 && (
              <div className="flex flex-col sm:flex-row sm:items-center justify-between pt-3 border-t border-border/40 gap-3">
                <div className="flex flex-wrap gap-1.5">
                  {[...selectedTiers, ...selectedPlatforms, ...selectedTypes].map((v) => (
                    <Badge key={v} variant="secondary" className="gap-1 text-xs rounded-full">
                      ...
                      <button
                        onClick={() => {
                          ...
                          setSelectedTypes((p) => p.filter((x) => x !== v));
                        }}
...
/* === CampaignBanner — uses useSearchParams, must be inside Suspense === */

function CampaignBanner() {
  const searchParams = useSearchParams();
  const router = useRouter();
  const { setTargetCampaignId } = useSelection();
  const addToCampaignId = searchParams.get("addTo");
  const { data: campaignsData } = useCampaigns();

  useEffect(() => {
    setTargetCampaignId(addToCampaignId);
  }, [addToCampaignId, setTargetCampaignId]);

  if (!addToCampaignId) return null;

  const campaign = campaignsData?.data?.find((c) => c.id === addToCampaignId);
    ...
    <div className="flex items-center gap-3 px-4 py-3 rounded-xl bg-primary/10 border border-primary/20">
      ...
      <p className="text-sm flex-1">
        <span className="font-medium">
          Adding KOLs to &quot;{campaign?.name ?? "Campaign"}&quot;
        </span>
        <span className="text-muted-foreground">
          {" "}
          — select KOLs below, then tap &quot;Add to Campaign&quot;
        </span>
      </p>
      <Button
        variant="ghost"
        ...
        onClick={() => {
          ...
          router.push(`/campaigns/${addToCampaignId}`);
        }}
      >
